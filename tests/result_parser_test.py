...
def testParseResults_always_yieldsValidVulnerabilities():
    ...
    with (pathlib.Path(__file__).parent / 'zap-test-output.json').open('r', encoding='utf-8') as o:
        ...
        parsed_vulnz = [v for v in vulnz if v.entry.title == 'Absence of Anti-CSRF Tokens']
        assert len(parsed_vulnz) == 11
        assert parsed_vulnz[0].entry.risk_rating == 4
        assert 'http://projects.webappsec.org/Cross-Site-Request-Forgery' in parsed_vulnz[0].entry.references
        assert 'Phase: Architecture and Design' in parsed_vulnz[0].entry.recommendation
        assert 'action="/search" method="GET" role="search">' in parsed_vulnz[0].technical_detail
